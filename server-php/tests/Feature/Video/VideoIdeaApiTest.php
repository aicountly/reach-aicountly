...
class VideoIdeaApiTest extends ApiTestCase
{
    ...
    {
        $resp = $this->authAs('super_admin')->call('GET', 'v1/video/ideas');
        $this->assertSame(200, $resp->response()->getStatusCode());
        $body = json_decode($resp->response()->getBody(), true);
        $this->assertArrayHasKey('data', $body);
    }

    public function testCreateIdeaReturns201(): void
    {
        $resp = $this->authAs('super_admin')->call('POST', 'v1/video/ideas', [
    ...
    {
        $create = $this->authAs('super_admin')->call('POST', 'v1/video/ideas', [
            'title'  => 'Read idea test',
            'status' => 'draft',
        ]);
        $uuid = json_decode($create->response()->getBody(), true)['data']['uuid'] ?? null;
        $this->assertNotNull($uuid, 'Create must return a uuid');

        $resp = $this->authAs('super_admin')->call('GET', "v1/video/ideas/{$uuid}");
        $this->assertSame(200, $resp->response()->getStatusCode());
    }

    public function testGetNonExistentIdeaReturns404(): void
    {
        $resp = $this->authAs('super_admin')->call('GET', 'v1/video/ideas/00000000-0000-0000-0000-000000000000');
    ...
    public function testAcceptIdeaReturns200(): void
    {
        ...
        $resp = $this->authAs('super_admin')->call('POST', "v1/video/ideas/{$uuid}/accept");
        $this->assertSame(200, $resp->response()->getStatusCode());
    }
    ...
    public function testConvertAcceptedIdeaCreatesProject(): void
    {
        ...
        $this->authAs('super_admin')->call('POST', "v1/video/ideas/{$uuid}/accept");

        $resp = $this->authAs('super_admin')->call('POST', "v1/video/ideas/{$uuid}/convert");
        $this->assertSame(201, $resp->response()->getStatusCode());
        $body = json_decode($resp->response()->getBody(), true);
        $this->assertArrayHasKey('data', $body);
        $this->assertNotEmpty($body['data']['uuid'] ?? '');
