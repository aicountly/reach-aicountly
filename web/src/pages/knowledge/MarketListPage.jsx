...
export function MarketListPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    knowledgeService.listMarkets({ page, limit, status, search })
  ...
  const columns = [
    { key: 'name', label: 'Market', render: (r) => (
      <div>
        ...
        <div className="text-xs text-muted">{r.region || r.slug}</div>
      </div>
    )},
    { key: 'jurisdiction_notes', label: 'Jurisdiction Notes', render: (r) => (
      <div className="text-sm" style={{ maxWidth: 300, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
        {r.jurisdiction_notes || '—'}
      </div>
    )},
    { key: 'country_codes', label: 'Countries', render: (r) => {
      const codes = Array.isArray(r.country_codes) ? r.country_codes : (typeof r.country_codes === 'string' ? JSON.parse(r.country_codes || '[]') : []);
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1>Markets</h1>
          <p className="text-sm text-muted">Geographic and regulatory markets for product positioning.</p>
        </div>
      </div>
      ...
      <FilterBar>
        ...
        <SearchBar value={search} onChange={(v) => { setSearch(v); setPage(1); }} placeholder="Search name / region…" />
