...
export function PersonaListPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    knowledgeService.listPersonas({ page, limit, status, search })
  ...
  const { isDeleting, error: deleteError, remove } = useRowDelete({
    onDelete: (r) => knowledgeService.deletePersona(r.id),
    onDone: load,
  });
  ...
  const columns = [
    { key: 'name', label: 'Name', render: (r) => (
      <div>
        ...
        <div className="text-xs text-muted">{r.role_title || r.slug}</div>
      </div>
    )},
    ...
    ...deleteColumn({
      canDelete: has('persona.manage'),
      isDeleting,
      remove,
      confirmMessage: (r) => `Delete persona “${r.name}”? This cannot be undone.`,
    }),
  ];
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1>Personas</h1>
          <p className="text-sm text-muted">Buyer and user personas mapped to products and features.</p>
        </div>
      </div>
      ...
      {loading ? <Loader /> : (
        <DataTable columns={columns} rows={rows} emptyMessage="No personas defined yet." />
      )}
