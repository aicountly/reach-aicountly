...
export function IndustryListPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    knowledgeService.listIndustries({ page, limit, status, search })
  ...
  const { isDeleting, error: deleteError, remove } = useRowDelete({
    onDelete: (r) => knowledgeService.deleteIndustry(r.id),
    onDone: load,
  });

  const columns = [
    { key: 'name', label: 'Industry', render: (r) => (
      <div>
    ...
    { key: 'description', label: 'Description', render: (r) => (
      <div className="text-sm" style={{ maxWidth: 360, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
        {r.description || '—'}
      </div>
    )},
    ...
    ...deleteColumn({
      canDelete: has('industry.manage'),
      isDeleting,
      remove,
      confirmMessage: (r) => `Delete industry “${r.name}”? This cannot be undone.`,
    }),
  ];
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1>Industries</h1>
          <p className="text-sm text-muted">Industry verticals that products and features serve.</p>
        </div>
      </div>
      ...
      <FilterBar>
        ...
        <SearchBar value={search} onChange={(v) => { setSearch(v); setPage(1); }} placeholder="Search name…" />
      ...
      {loading ? <Loader /> : (
        <DataTable columns={columns} rows={rows} emptyMessage="No industries defined yet." />
      )}
